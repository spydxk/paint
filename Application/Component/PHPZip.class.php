...
class PHPZip
{
    ...
    private function ReadCentralFileHeaders($zip, $i = 0)
    {
        $binary_data = fread($zip, 46);
        $header      = unpack('vchkid/vid/vversion/vversion_extracted/vflag/vcompression/vmtime/vmdate/Vcrc/Vcompressed_size/Vsize/vfilename_len/vextra_len/vcomment_len/vdisk/vinternal/Vexternal/Voffset', $binary_data);

        $header['filename'] = ($header['filename_len'] != 0) ? fread($zip, $header['filename_len']) : '';
        $encode             = mb_detect_encoding($header['filename'], array("ASCII", 'UTF-8', "GB2312", "GBK", 'BIG5'));
        $header['filename'] = mb_convert_encoding($header['filename'], 'UTF-8', $encode); //
    ...
    //$rst = $archive->unZip($zipfile, $savepath, $i);
    //                   压缩包    保存路径
    public function unZip($zipfile, $to, $index = array(-1))
    {
        ...
        for ($i = 0; $i < $cdir['entries']; $i++) {
            @fseek($zip, $pos_entry);
            $header          = $this->ReadCentralFileHeaders($zip, $i);
            $header['index'] = $i;
            $pos_entry       = ftell($zip);
            @rewind($zip);
            fseek($zip, $header['offset']);
            if (in_array("-1", $index) || in_array($i, $index)) {
                $res = $this->ExtractFile($header, $to, $zip, $i);
            }
        }
        fclose($zip);
        $stat['filename'] = $res;
    ...
    private function ExtractFile($header, $to, $zip, $index)
    {
        ...
        $time = date('Ymd', time()).(time() + $index * 2);
        ...
        if (!($header['external'] == 0x41FF0010) && !($header['external'] == 16)) {
            if ($header['compression'] == 0) {
                ...
                $etc = pathinfo($header['filename'], PATHINFO_EXTENSION);
                $tte = rename($to . $header['filename'], $to . $time . '.' . $etc);
            } else {
